...
// 画像の表に基づいた100個の感情データ (縦:エネルギー10〜1, 横:フィーリング1〜10)
const emotions = {
    // 高エネルギー・不快 (左上: 赤)
...
for (let y = 10; y >= 1; y--) {
    for (let x = 1; x <= 10; x++) {
        ...
        // 色の設定
        if (x <= 5 && y > 5) cell.style.backgroundColor = `hsl(0, 60%, ${90 - (y-5)*7}%)`;
        else if (x > 5 && y > 5) cell.style.backgroundColor = `hsl(30, 70%, ${90 - (y-5)*7}%)`;
        else if (x <= 5 && y <= 5) cell.style.backgroundColor = `hsl(210, 50%, ${90 - (5-y)*7}%)`;
        else cell.style.backgroundColor = `hsl(120, 40%, ${90 - (5-y)*7}%)`;
        ...
        cell.onclick = () => {
            ...
            // 選択時のフィードバック
            document.querySelectorAll('.cell').forEach(c => c.style.border = "1px solid rgba(0,0,0,0.05)");
