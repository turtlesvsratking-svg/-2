...
for (let y = 10; y >= 1; y--) {
    for (let x = 1; x <= 10; x++) {
        ...
        let baseColor = "";
        if (x <= 5 && y > 5) baseColor = `hsl(0, 75%, ${90 - (y-5)*6}%)`;      // 赤
        else if (x > 5 && y > 5) baseColor = `hsl(40, 90%, ${90 - (y-5)*6}%)`;  // 黄
        else if (x <= 5 && y <= 5) baseColor = `hsl(210, 60%, ${95 - (5-y)*6}%)`; // 青
        else baseColor = `hsl(140, 50%, ${90 - (5-y)*6}%)`;                     // 緑
        ...
        cell.onclick = () => {
            ...
            if (data) {
                currentSelection = { jp: data[0], en: data[1], color: baseColor };
...
saveBtn.onclick = () => {
    ...
    const logEntry = {
        ...
        emotion: currentSelection.jp,
        color: currentSelection.color,
        memo: memoInput.value || "（メモなし）"
    };
...
function renderHistory() {
    ...
    historyList.innerHTML = logs.map(log => `
        <div class="history-item" style="border-left-color: ${log.color}">
            <div class="history-content">
                <div class="time">${log.date}</div>
                <div class="emotion-name">${log.emotion}</div>
